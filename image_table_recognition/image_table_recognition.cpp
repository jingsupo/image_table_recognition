...
private:
    ...
    int params_line_x = 20;     //x上点个数的差值调节（线不均匀，有的粗有的细，甚至有的不连续）
    int params_line_y = 20;     //y上点个数的差值调节（线不均匀，有的粗有的细，甚至有的不连续）
...
private:
    ...
    std::pair<std::vector<int>, std::vector<int>> _get_xy(std::vector<int> coords, int params_margin)
    {
        ...
        {
            if (sorted_coords[i + 1] - sorted_coords[i] > params_margin) coord_list.push_back(sorted_coords[i]);
        }
...
std::vector<std::vector<int>> image_table_recognition::run(std::string img_path, std::string out_path)
{
    ...
    for (int i = 0; i < list_y.size(); i++)
    {
        ...
        {
            if ((*coord)[0] == list_y[i]) line_list.push_back(*coord);
        }
    ...
    for (int i = 0; i < coord_map.size() - 1; i++)
    {
        ...
        {
            if (index == coord_map[i].size() - 1) break;
...
int main()
{
    ...
    auto ret = itr.run("qiaofeng.png", "");
    std::cout << "单元格数量：" << ret.size() << "\n";
    std::for_each(ret.begin(), ret.end(), [](std::vector<int> v) { std::cout << "单元格坐标：" << v[0] << " " << v[1] << " " << v[2] << " " << v[3] << "\n"; });
    system("pause");
}
